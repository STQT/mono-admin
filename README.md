...
Бэкенд живёт в соседнем репо `../mona` и задеплоен на **https://aksiya.monoelectric.uz**. Эндпоинты под Next.js admin собраны в `core/admin_api/` и доступны под префиксом `/api/admin/`:
...
1. Скопировать env и проверить URL backend:
   ...
   # NEXT_PUBLIC_API_BASE_URL=https://aksiya.monoelectric.uz
   ```
